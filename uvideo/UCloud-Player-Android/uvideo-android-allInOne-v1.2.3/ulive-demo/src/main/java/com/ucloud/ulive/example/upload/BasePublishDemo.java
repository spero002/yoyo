    ...
    public void onStateChanged(int type, Object event) {
       switch (type) {
            ...
            case UEasyStreaming.State.MEDIA_MUXER_PREPARED_ERROR:
                Log.e(TAG, "prepare error, the publish stream id may be reuse, server error, try change one.");
                Toast.makeText(this, "the publish stream id may be reuse, server error, try change one.", Toast.LENGTH_LONG).show();
                break;
